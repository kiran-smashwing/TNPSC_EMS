...
class ProcessUpdateAdditionalCandidatesCsv implements ShouldQueue
{
    ...
    {
        $failedCsvPath = 'failes_csv/' . $this->examId . '_addl_cand_count_failed_rows_' . time() . '.csv';
    ...
    private function sendProcessingResultEmail($status, $successfulInserts, $failedCount, $errorMessage, $failedCsvPath)
    {
        ...
        $data = [
            'taskType' => 'Additional Candidates Count Upload',
            'status' => $status,
    ...
    private function validateAndInsertRow($data, $examId, &$successfulInserts, &$failedRows)
    {
        ...
        // Update the record
        if ($confirmedByID) {
            $confirmedByID->addl_cand_count = $candidatesCount;
            $confirmedByID->save();
        } else {
    ...
    {
        $failedCsvFiles = glob(storage_path('app/public/failes_csv/' . $examId . '_addl_cand_count_failed_rows_*.csv'));
    ...
    private function logUploadAction( $examId, $successfulInserts, $failedCount, $uploadedFileUrl, $failedCsvPath, $currentUser)
    {
        ...
        if ($existingLog) {
            ...
            $this->auditService->updateLog(
                ...
                metadata: $metadata,
                afterState: null,
                description: 'Updated Additional Candidates Count CSV'
            );
        } else {
            ...
            $this->auditService->log(
                ...
                afterState: null,
                description: 'Uploaded Additional Candidates Count CSV',
                metadata: $metadata
            );
        }
    }
