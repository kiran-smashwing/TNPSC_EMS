                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="d-sm-flex align-items-center justify-content-between">
                                <h5 class="mb-3 mb-sm-0">CI Check list</h5>
                                <div>
                                    <a href="{{ route('ci-checklist.create') }}" class="btn btn-outline-success">Add CI
                                        Checklist</a>
                                </div>
                            </div>
                        </div>
                                ...
                                <tbody>
                                    @foreach ($ciChecklists as $index => $ciChecklist)
                                        <tr>
                                            ...
                                            <!-- Actions -->
                                            <td>
                                                <a href="{{ route('ci-checklist.edit', $ciChecklist->ci_checklist_id) }}"
                                                    class="avtar avtar-xs btn-light-success">
                                                    <i class="ti ti-edit f-20"></i>
                                                </a>
                                                <a href="#"
                                                    class="avtar avtar-xs status-toggle {{ $ciChecklist->ci_checklist_status ? 'btn-light-success' : 'btn-light-danger' }}"
                                                    data-ci-checklist-id="{{ $ciChecklist->ci_checklist_id }}"
                                                    title="Change Status (Active or Inactive)">
                                                    <i
                                                        class="ti ti-toggle-{{ $ciChecklist->ci_checklist_status ? 'right' : 'left' }} f-20"></i>
                                                </a>
                                            </td>
                                        </tr>
                                    @endforeach
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.body.addEventListener('click', function(e) {
                    ...
                    // Get the CiChecklist ID from the data attribute
                    const ciChecklistId = button.dataset.ciChecklistId;

                    // Send the request to toggle the status
                    fetch(`{{ url('/ci-checklist') }}/${ciChecklistId}/toggle-status`, {
                            method: 'POST',
                            headers: {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                showNotification(
                                    'Status Updated',
                                    data.message || 'CiChecklist status updated successfully',
                                    'success'
                                );
                            } else {
