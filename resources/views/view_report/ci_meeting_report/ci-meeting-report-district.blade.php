        ...
        @foreach ($grouped_data as $district_name => $data)
            <div class="district-section" style="{{ !$loop->first ? 'page-break-before: always;' : '' }}">
                    ...
                    <tbody>
                        @foreach ($data['ci_meeting_records'] as $index => $record)
                            <tr>
                                ...
                                <td>{{ $record->created_at ? $record->created_at->format('d-m-Y h:i:s A') : 'N/A' }}</td>
                                <td>{{ $record->updated_at ? $record->updated_at->format('d-m-Y h:i:s A') : 'N/A' }}</td>
                ...
                @php
                    ...
                    $scannedCIs = collect($data['ci_meeting_records'])->filter(fn($record) => $record->updated_at !== null)->count();
